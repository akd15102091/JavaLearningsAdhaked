...
import DesignSplitWiseLLD.splitstrategy.SplitStrategy;
import DesignSplitWiseLLD.splitstrategy.SplitStrategyFactory;
...
public class SplitWiseService {
    ...
    public Group createGroup(String name, List<String> userIds){
        List<User> members = userIds.stream().map(users::get).collect(Collectors.toList());
    ...
    public Expense addExpense(String description, double amount, String paidByUserId, String groupId,
        ...
        Expense expense = new Expense.Builder().description(description).amount(amount).paidBy(paidBy).group(group).addSplits(splits).build();
